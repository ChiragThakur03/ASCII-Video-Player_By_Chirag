...
T = {
    "app_name": "ASCII Player By Chirag Bahadur",
...
def get_logo_str():
    return f"""
{C_MAGENTA}    ╔═════════════════════════════════════════════════════════════════════════════════════════════╗
    ║  {C_BOLD}{C_YELLOW} █████╗ ███████╗ ██████╗██╗██╗     ██████╗ ██╗      █████╗ ██╗   ██╗███████╗██████╗  {C_MAGENTA} ║
    ║  {C_BOLD}{C_YELLOW}██╔══██╗██╔════╝██╔════╝██║██║     ██╔══██╗██║     ██╔══██╗╚██╗ ██╔╝██╔════╝██╔══██╗ {C_MAGENTA} ║
    ║  {C_BOLD}{C_YELLOW}███████║███████╗██║     ██║██║     ██████╔╝██║     ███████║ ╚████╔╝ █████╗  ██████╔╝ {C_MAGENTA} ║
    ║  {C_BOLD}{C_YELLOW}██╔══██║╚════██║██║     ██║██║     ██╔═══╝ ██║     ██╔══██║  ╚██╔╝  ██╔══╝  ██╔══██╗ {C_MAGENTA} ║
    ║  {C_BOLD}{C_YELLOW}██║  ██║███████║╚██████╗██║██║     ██║     ███████╗██║  ██║   ██║   ███████╗██║  ██║ {C_MAGENTA} ║
    ║  {C_BOLD}{C_YELLOW}╚═╝  ╚═╝╚══════╝ ╚═════╝╚═╝╚═╝     ╚═╝     ╚══════╝╚═╝  ╚═╝   ╚═╝   ╚══════╝╚═╝  ╚═╝ {C_MAGENTA} ║
    ║                                {C_BOLD}{C_CYAN}By C H I R A G   B A H A D U R{C_MAGENTA}                               ║
    ╚═════════════════════════════════════════════════════════════════════════════════════════════╝{RESET_COLOR}"""
...
def export_flow(video_path, use_color, width):
    clear_console()
    print(f"\n  {C_BOLD}{C_GREEN}» EXPORTATION TO MP4 «{RESET_COLOR}")
    ...
    while True:
        ...
        try:
            while True:
                ...
                # Print art only (no logo) to use full space
...
def main():
    ...
    if len(sys.argv) < 2:
        show_logo()
        print(f"  {C_RED}Usage: python ASCII_V1.py <video_filename>{RESET_COLOR}")
        print(f"  Example: python ASCII_V1.py video.mp4")
        sys.exit(1)
        
    vid = sys.argv[1]
    if not os.path.exists(vid):
        show_logo()
        print(f"  {C_RED}File not found: '{vid}'{RESET_COLOR}")
        sys.exit(1)

    while True:
        show_logo()
        print(f"  {C_BOLD}{C_YELLOW}» {T['app_name']} «{RESET_COLOR}\n")
        print(f"  {C_BOLD}{C_GREEN}»{RESET_COLOR} Video: {vid}")
        
        use_color = True # Always colored
